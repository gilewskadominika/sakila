...
query_insert_user_table = sql.SQL("""
    INSERT INTO {table_name}(name, email, password)
    VALUES (%s,%s,%s)
""").format(table_name=sql.Identifier('User'))

query_update_user_table = sql.SQL("""
    UPDATE {table_name} SET password=%s WHERE id = %s
""").format(table_name=sql.Identifier('User'))

query_delete_user_table = sql.SQL("""
...
query_create_db = sql.SQL("""
    CREATE DATABASE alegrosz2
""").format(table_name=sql.Identifier('User'))
...
#         print(error)
#     try:
#         cursor.execute(query_insert_user_table, ('Ala', 'a@a.pl', 'michal_dworczyk2024'))
#     except DatabaseError as error:
#         print(error)
#     try:
#         cursor.execute(query_update_user_table, ('topSecretMateusz2024', 1))
#     except DatabaseError as error:
#         print(error)
#     try:
#         cursor.execute(query_delete_user_table, ('Ala',))  # tu musi być tupla, więc jak jeden element, to musi być ','
#     except DatabaseError as error:
#         print(error)
#     try:
